...
public abstract class VytrackPageBase {
    ...
    public void changeMenu(String menu1, String menu2) {
        //menu1=Fleet, menu2 = Vehicles
        //1. locate menu1 and make it dynamic
        String menu1X = "//span[contains(text(), '"+menu1+"')][@class='title title-level-1']";
        WebElement menuOne = Driver.getDriver().findElement(By.xpath(menu1X));

        String menu2X = "//span[.='"+menu2+"'][@class='title title-level-2']";
        WebElement menuTwo = Driver.getDriver().findElement(By.xpath(menu2X));

        WebDriverWait wait = new WebDriverWait(Driver.getDriver(), 5);
        wait.until(ExpectedConditions.elementToBeClickable(menuOne));
        menuOne.click();
        try {
            Thread.sleep(1000);
